...
包装器、启动参数）、启动前自愈、以及无界面的自举器都已实现。

在面向小白的「下载即双击可用」版本发布前，还差两件事：

- 一个托管的 NeatWhisky Wine 11.x 构建（Wine + DXVK + MoltenVK，不含 GPTK/CrossOver）；
- 一个经 Developer ID 签名并公证的发布版（CI 流水线已就绪）。
