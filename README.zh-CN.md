...
NeatWhisky 目前通过 Homebrew Cask 分发（面向小白的「双击即用」签名 DMG 待 Apple Developer ID 签名就绪后推出）。

```bash
brew install --cask shawnsayno/tap/neatwhisky
```

cask 在安装时会自动剥离 macOS 隔离属性，首次打开即可直接启动，无需额外操作。安装后在启动台或 `/Applications/NeatWhisky.app` 找到它。

## 当前状态

「一键」体验已经建好：开箱向导、修复引擎、Steam 配方（CJK 字体、黑屏 + 重启循环
包装器、启动参数）、启动前自愈、以及无界面的自举器都已实现。托管的 NeatWhisky
Wine 11.x 构建（Wine + DXVK + MoltenVK，不含 GPTK/CrossOver）已就绪，CI 会在每次
发布时构建并上传 DMG。

当前通过 Homebrew Cask 分发（见[安装](#安装)）。面向完全不用命令行的小白的
「Developer ID 签名 + 公证、双击即用」DMG，待 Apple 签名配置好后推出——相关 CI
流水线已就绪。

构建计划见 [`docs/`](docs/)，技术原理详解见
...
Fork 血缘与相对上游的改动清单见 [`NOTICE.md`](docs/NOTICE.md)。
